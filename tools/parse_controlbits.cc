...
auto load_controlbits_new(const std::FilePath& path, int mode) -> std::Option<Controlbits> 
auto parse_line_new(std::String line) -> std::Tuple<std::String, std::Vector<std::String>>
auto parse_cob_new(const std::Vector<std::String>& tokens, Controlbits& controlbits, const std::String& reversed_bits) -> void
auto parse_tob_new(
...
auto load_controlbits_new(const std::FilePath& path, int mode) -> std::Option<Controlbits> {
    try {
        ...
        while (std::getline(file, line)) {
            ...
            auto [reversed_bits, tokens] = parse_line_new(line);

            if (tokens[0] == "cob") {
                parse_cob_new(tokens, controlbits, reversed_bits);
                ...
                auto has_value = parse_tob_new(tokens, controlbits, reversed_bits, sig_arrays);
                if (!has_value) 
                    continue;
            }
        }
    
        process_bump_sig_new(controlbits, tob2bump_sig, bump2tob_sig);
        process_track_sig_new(controlbits, tob2track_sig, track2tob_sig);
...
try {
    ...
    if (tokens[3] == "dly" || tokens[3] == "drv") {
        ...
        kiwi::parse::setBits<128>(iter.first->second, index, 32, reversed_bits);
        ...
        for (int i = 0; i < 24; i += 3) {
            // int value = std::stoi(reversed_bits.substr(i, 3), nullptr, 2);     这是最高位在左边的做法
        ...
        kiwi::parse::setBits<64>(iter.first->second, index, 32, reversed_bits);
    }

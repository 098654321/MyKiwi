...
auto route_bump_to_bump_net_multi_mode(
    hardware::Interposer* interposer, circuit::BumpToBumpNet* net, algo::OccupancyView& view, HardwareRecorder& recorder, int mode
) -> bool {
...
try {
    ...
    // begin/end nodes
    auto begins_map = searching_points_multi_mode<hardware::Bump>(begin_bump, net, interposer, view, mode);
    auto ends_map = searching_points_multi_mode<hardware::Bump>(end_bump, net, interposer, view, mode);
    ...
    path_package._regular_path = route_path_multi_mode(
        interposer, 
        map_to_vec_multi_mode<hardware::Bump>(begin_bump, begins_map, recorder, reuse_type.value()), map_to_set_multi_mode(ends_map),
        std::HashSet<hardware::Track*>{}, view, recorder, reuse_type.value(), mode
    );

    // set
    auto head = std::get<0>(path_package._regular_path.front());
    auto tail = std::get<0>(path_package._regular_path.back());
    set_tobconnector_multi_mode(begins_map, head, begin_bump, path_package, true);
    set_tobconnector_multi_mode(ends_map, tail, end_bump, path_package, false);
    ...
    debug::info_fmt("route_bump_to_bump_net: {}", err.what());
    path_package.reset_all();
    return false;
}
}

auto route_track_to_bump_net_multi_mode(
    hardware::Interposer* interposer, circuit::TrackToBumpNet* net, algo::OccupancyView& view, HardwareRecorder& recorder, int mode
) -> bool {
    circuit::PathPackage path_package {};
try {
    ...
    auto begins_map = searching_points_multi_mode<hardware::Track>(begin_track, net, interposer, view, mode);
    auto ends_map = searching_points_multi_mode<hardware::Bump>(end_bump, net, interposer, view, mode);
    ...
    path_package._regular_path = route_path_multi_mode(
        interposer, 
        map_to_vec_multi_mode<hardware::Track>(begin_track, begins_map, recorder, reuse_type.value()), map_to_set_multi_mode(ends_map),
        std::HashSet<hardware::Track*>{}, view, recorder, reuse_type.value(), mode
    );
    ...
    debug::info_fmt("route_track_to_bump_net: {}", err.what());
    path_package.reset_all();
    return false;
}
}

auto route_bump_to_track_net_multi_mode(
    hardware::Interposer* interposer, circuit::BumpToTrackNet* net, algo::OccupancyView& view, HardwareRecorder& recorder, int mode
) -> bool {
    circuit::PathPackage path_package {};
try {
    auto begin_bump = net->begin_bump();
    ...
    auto begins_map = searching_points_multi_mode<hardware::Bump>(begin_bump, net, interposer, view, mode);
    auto ends_map = searching_points_multi_mode<hardware::Track>(end_track, net, interposer, view, mode);
    ...
    debug::info_fmt("route_bump_to_track_net: {}", err.what());
    path_package.reset_all();
    return false;
}
}

auto route_bump_to_bumps_net_multi_mode(
    hardware::Interposer* interposer, circuit::BumpToBumpsNet* net, algo::OccupancyView& view, HardwareRecorder& recorder, int mode
) -> bool {
    circuit::PathPackage path_package {};
try {
    auto begin_bump = net->begin_bump();
    ...
    for (auto end_bump: end_bumps) {
        auto current_begin_map = searching_points_multi_mode<hardware::Bump>(begin_bump, net, interposer, view, mode);
        for (auto& [track, connector]: total_regular_path) {
            current_begin_map.emplace(track, std::nullopt);
        }
        auto current_begin_tracks = map_to_vec_multi_mode<hardware::Bump>(begin_bump, current_begin_map, recorder, net->reuse_type().value());

        auto ends_map = searching_points_multi_mode<hardware::Bump>(end_bump, net, interposer, view, mode);
        auto current_end_tracks = map_to_set_multi_mode(ends_map);

        auto regular_path = route_path_multi_mode(
            interposer, current_begin_tracks, current_end_tracks, std::HashSet<hardware::Track*>{},
            view, recorder, net->reuse_type().value(), mode
        );
        ...
        set_tobconnector_multi_mode(current_begin_map, head, begin_bump, path_package, true);
        set_tobconnector_multi_mode(ends_map, tail, end_bump, path_package, false);
    ...
    debug::info_fmt("route_bump_to_bumps_net: {}", err.what());
    path_package.reset_all();
    return false;
}
}

auto route_track_to_bumps_net_multi_mode(
    hardware::Interposer* interposer, circuit::TrackToBumpsNet* net, algo::OccupancyView& view, HardwareRecorder& recorder, int mode
) -> bool {
    circuit::PathPackage path_package {};
try {
    auto begin_track = net->begin_track();
    ...
    algo::routed_path total_regular_path {};
    for (auto& end_bump: end_bumps) {
        auto current_begin_map = searching_points_multi_mode<hardware::Track>(begin_track, net, interposer, view, mode);
        for (auto& [track, connector]: total_regular_path) {
            current_begin_map.emplace(track, std::nullopt);
        }
        auto current_begin_tracks = map_to_vec_multi_mode<hardware::Track>(begin_track, current_begin_map, recorder, net->reuse_type().value());

        auto end_map = searching_points_multi_mode<hardware::Bump>(end_bump, net, interposer, view, mode);
        auto current_end_tracks = map_to_set_multi_mode(end_map);
        ...
        auto tail = std::get<0>(regular_path.back());
        set_tobconnector_multi_mode(end_map, tail, end_bump, path_package, false);
        path_package._length += path_length(regular_path) - 1;
    }
    path_package._regular_path = total_regular_path;
    path_package._length += 1;
    ...
    debug::info_fmt("route_track_to_bumps_net: {}", err.what());
    path_package.reset_all();
    return false;
}
}

auto route_bump_to_tracks_net_multi_mode(
    hardware::Interposer* interposer, circuit::BumpToTracksNet* net, algo::OccupancyView& view, HardwareRecorder& recorder, int mode
) -> bool {
    circuit::PathPackage path_package {};
try {
    auto begin_bump = net->begin_bump();
    ...
    for (auto& end_track: end_tracks) {
        ...
        auto end_map = searching_points_multi_mode<hardware::Track>(end_track, net, interposer, view, mode);
        auto current_end_tracks = map_to_set_multi_mode(end_map);
...
auto route_tracks_to_bumps_net_multi_mode(
    hardware::Interposer* interposer, circuit::TracksToBumpsNet* net, algo::OccupancyView& view, HardwareRecorder& recorder, int mode
) -> bool {
    circuit::PathPackage path_package {};
try {
    ...
    for (auto& end_bump: end_bumps) {
        std::sort(begin_tracks.begin(), begin_tracks.end(), [&](auto& t1, auto& t2){
            return recorder.track_cost(t1, mode) < recorder.track_cost(t2, mode);
        });
        auto ends_map = searching_points_multi_mode<hardware::Bump>(end_bump, net, interposer, view, mode);

        auto regular_path = route_path_multi_mode(
            interposer, begin_tracks, map_to_set_multi_mode(ends_map), std::HashSet<hardware::Track*>{},
            view, recorder, net->reuse_type().value(), mode
        );
        // remove repeated tracks
        regular_path.erase(std::unique(regular_path.begin(), regular_path.end(), [](const auto& a, const auto& b) {
...
auto route_sync_net_multi_mode(
    hardware::Interposer* interposer, circuit::SyncNet* sync_net, algo::OccupancyView& view, HardwareRecorder& recorder, int mode
) -> bool {
try {
    ...
    if (sync_net->btbnets().size() > 0){
        auto current_len = sync_preroute_bump_to_bump_multi_mode(
            interposer, sync_net->btbnets(), occupied_tracks_vec, view, recorder, mode
        );
        max_length = current_len > max_length ? current_len : max_length;
    }
    if (sync_net->ttbnets().size() > 0){
        auto current_len = sync_preroute_track_to_bump_multi_mode(
            interposer, sync_net->ttbnets(), occupied_tracks_vec, view, recorder, mode
        );
        max_length = current_len > max_length ? current_len : max_length;
    }
    if (sync_net->bttnets().size() > 0){
        auto current_len = sync_preroute_bump_to_track_multi_mode(
            interposer, sync_net->bttnets(), occupied_tracks_vec, view, recorder, mode
        );
        max_length = current_len > max_length ? current_len : max_length;
    }
    ...
    while (true){
        auto [success, ml] = sync_incremental_reroute_multi_mode(
            interposer, btb_nets, max_length, view, recorder, mode
        );
        if (success){
            auto [success, ml] = sync_incremental_reroute_multi_mode(
                interposer, ttb_nets, max_length, view, recorder, mode
            );
            if (success){
                auto [success, ml] = sync_incremental_reroute_multi_mode(
                    interposer, btt_nets, max_length, view, recorder, mode
                );
                max_length = ml;
                if (success){
...
auto sync_preroute_bump_to_bump_multi_mode(
    ...
    if (sync_net.size() <= 0){
        throw std::runtime_error("sync_preroute_bump_to_bump(): empty sync_net");
    }

    for (auto& net: sync_net) {
        auto begin_bump = net->begin_bump();
        auto end_bump  =net->end_bump();
        debug::check(begin_bump->tob() != end_bump->tob(), "preroute(): Route bump in the same tob");

        auto begin_map = view.available_tracks_bump_to_track(begin_bump, mode);
        auto begin_track_vec = map_to_vec_multi_mode<hardware::Bump>(begin_bump, begin_map, recorder, net->reuse_type().value());
        auto end_map = view.available_tracks_track_to_bump(end_bump, mode);
        auto end_track_set = map_to_set_multi_mode(end_map);
        ...
        if (!begin_map.contains(head)) {
            throw std::runtime_error("sync_preroute_bump_to_bump(): head of path not in begin_map");
        }
        ...
        if (!end_map.contains(tail)) {
            throw std::runtime_error("sync_preroute_bump_to_bump(): tail of path not in end_map");
        }
...
auto sync_preroute_bump_to_track_multi_mode(
    ...
    if (sync_net.size() <= 0){
        throw std::runtime_error("sync_preroute_bump_to_track(): empty sync_net");
    }
    ...
    for (auto& net: sync_net) {
        ...
        auto head = std::get<0>(path_package._regular_path.front());
        if (!begin_map.contains(head)) {
            throw std::runtime_error("sync_preroute_bump_to_track(): head of path not in begin_map");
        }
        auto begin_tobconnector = begin_map.find(head)->second;
        path_package._tob_to_track.emplace_back(
            std::Tuple<hardware::Bump*, hardware::TOBConnector, hardware::Track*>(begin_bump, begin_tobconnector, head)
...
auto sync_preroute_track_to_bump_multi_mode(
    ...
    if (sync_net.size() <= 0){
        throw std::runtime_error("sync_preroute_track_to_bump(): empty sync_net");
    }
    ...
    for (auto& net: sync_net) {
        ...
        auto end_map = view.available_tracks_track_to_bump(end_bump, mode);
        auto end_set = map_to_set_multi_mode(end_map);
        ...
        auto tail = std::get<0>(path_package._regular_path.back());
        if (!end_map.contains(tail)) {
            throw std::runtime_error("sync_preroute_track_to_bump(): tail of path not in end_map");
        }
        auto end_tobconnector = end_map.find(tail)->second;
        path_package._track_to_tob.emplace_back(
            std::Tuple<hardware::Bump*, hardware::TOBConnector, hardware::Track*>(end_bump, end_tobconnector, tail)
...
auto sync_incremental_reroute_multi_mode(
    ...
    if (nets_to_be_rerouted.size() > 0) {
        MazeRerouter rerouter{true};
        rerouter.set_recorder(&recorder);
        auto [success, ml] = rerouter.bus_reroute(interposer, nets_to_be_rerouted, max_length, view, mode);
...
auto route_path_multi_mode(
    ...
    auto path_info = maze_search_multi_mode(interposer, begin_tracks, end_tracks, occupied_tracks, view, recorder, reuse_type, mode);  // negative sequence
...
auto maze_search_multi_mode(
    ...
    while (!queue.empty()) {
        ...
        for (auto& [next_track, connector] : view.adjacent_idle_tracks(mode, track)) {
...
auto searching_points_multi_mode(
    Node* node, circuit::Net* net, hardware::Interposer* interposer, const algo::OccupancyView& view, int mode
    ...
    static_assert(
        ...
        "searching_points_multi_mode(): Node must be hardware::Bump or hardware::Track"
    );

    auto map = std::HashMap<hardware::Track*, std::Option<hardware::TOBConnector>>{};
    if constexpr(std::is_same<Node, hardware::Bump>::value) {
        auto track_and_connector = view.available_tracks_bump_to_track(node, mode);
    ...
    if (map.empty()) {
        throw std::runtime_error("searching_points_multi_mode(): no available tracks");
    }
...
auto set_tobconnector_multi_mode(
    ...
    if (iter == map.end()) {
        throw std::runtime_error("set_tobconnector_multi_mode(): cannot find track in tracks_map");
    }
    ...
    if (iter->second.has_value()) {
        ...
        debug::debug(std::format("set_tobconnector_multi_mode: track {} connect to an existing net_path", track->coord()));
    }
}
...
auto map_to_vec_multi_mode(
    ...
    static_assert(
        std::is_same<Node, hardware::Bump>::value || std::is_same<Node, hardware::Track>::value,\
        "map_to_vec_multi_mode(): Node must be hardware::Bump or hardware::Track"
    );

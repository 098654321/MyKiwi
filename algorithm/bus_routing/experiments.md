...
## 关于算法
0. 当 bump -> track 的映射确定之后，就可以判断两条线是否位于同一个 cobunit 。对于不同 cobunit 的线，其 track -> track 的布线过程完全没有冲突，可以并行，而且不需要检查数据冲突

1. 从 bump 到 track 的过程中对于 mux 的使用冲突是造成资源冲突的主要原因。可以尝试将这一过程与 track 上的迷宫搜索过程分开做，对这一过程用单独优化
...
4. 初步尝试发现增大重布线的时候的拆线比例可用提高重连成功率，但是线长会增加。可做一下实验，用case12

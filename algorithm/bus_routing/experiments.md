...
## VPR
  ...
  - 经典的开源 FPGA 布线工具，其中实现了 PathFinder 

## 关于算法

1. 还有布线顺序对布线成功率有影响，可以在RetryExpt中加入应对策略 （能不能把这个看作增量布线的一部分）
  - 如果非同步线布线失败，可以把同步线不动，非同步线拆了，然后挂掉的那个先连。

2. 从 bump 到 track 的过程中对于 mux 的使用冲突是造成资源冲突的主要原因。可以尝试将这一过程与 track 上的迷宫搜索过程分开做，对这一过程用单独优化

  这个冲突有两个原因。一个是同一个 tob 上的 bump 相互竞争 mux，另一个是位于不同 tob 但是属于同一根 net 的 bump 相互限制

  最关键的地方：当 bump 和目标 track 确定之后中间的整个 mux 路径也确定了，整个路径唯一对应，可以根据 bump 和 track 的 index 唯一的计算出来。还有一个限制，即同一组的 bump 不能分到同一组（ 8 个一组，共 16 组）的 track ，不然会抢 hori mux

  allocator 部分把这个问题建模为**二分图的最大匹配问题**，求解 bump 集合的最大匹配，并且可以明确知道是否所有 bump 都能匹配上。**但是这么建模不太对，因为忽略了组号的限制。该过程可以分为两步：先把 bump 分配到某个组，再到组内分配具体的 track**

  算法：
  - 匈牙利算法，适合小规模二分图
  - Hopcroft-Karp 算法，适合更大规模的二分图**可用这个**
  
3. 当 bump -> track 的映射确定之后，就可以判断两条线是否位于同一个 cobunit 。对于不同 cobunit 的线，其 track -> track 的布线过程完全没有冲突，可以并行，而且不需要检查数据冲突


4. [VPR](https://github.com/verilog-to-routing/vtr-verilog-to-routing)

5. 初步尝试发现增大重布线的时候的拆线比例可用提高重连成功率，但是线长会增加。可做一下实验，用case12
